...
class NJIDECopier {
    ...
    setupFloatingUI() {
        ...
        style.textContent = `
            ...
            .nj-header { background: rgba(0,0,0,0.2); padding: 12px 16px; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid rgba(255,255,255,0.1); }
            .status-indicator { width: 10px; height: 10px; border-radius: 50%; background: #666; animation: pulse 2s infinite; }
            .status-indicator.connected { background: #4CAF50; }
            .status-indicator.disconnected { background: #f44336; }
            .status-indicator.error { background: #FF9800; }
            @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }
            .nj-title { font-weight: 700; font-size: 14px; flex: 1; }
            .nj-platform { font-size: 11px; opacity: 0.8; background: rgba(255,255,255,0.2); padding: 2px 8px; border-radius: 10px; }
            .nj-section { padding: 12px 16px; border-bottom: 1px solid rgba(255,255,255,0.1); }
            .nj-section-title { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; opacity: 0.8; margin-bottom: 10px; }
            .nj-btn { width: 100%; padding: 10px 14px; margin-bottom: 8px; background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.2); border-radius: 8px; color: white; cursor: pointer; font-size: 13px; display: flex; align-items: center; gap: 10px; transition: all 0.2s; text-align: left; }
            .nj-btn:last-child { margin-bottom: 0; }
            .nj-btn:hover { background: rgba(255,255,255,0.25); transform: translateX(4px); }
            .nj-btn:active { transform: translateX(2px); }
            .nj-btn-primary { background: #4CAF50; border-color: #4CAF50; }
            .nj-btn-primary:hover { background: #45a049; }
            .nj-btn-secondary { background: #2196F3; border-color: #2196F3; }
            .nj-btn-secondary:hover { background: #1976D2; }
            .nj-btn-export { background: #9C27B0; border-color: #9C27B0; }
            .nj-btn-export:hover { background: #7B1FA2; }
            .nj-btn-stats { background: #FF9800; border-color: #FF9800; }
            .nj-btn-stats:hover { background: #F57C00; }
            .btn-icon { font-size: 16px; width: 24px; text-align: center; }
            .btn-text { flex: 1; }
            .nj-status-bar { padding: 8px 16px; background: rgba(0,0,0,0.2); font-size: 11px; text-align: center; }
            .nj-status-text { opacity: 0.9; }
        `;
    ...
    async handleAction(action) {
        switch(action) {
            case 'copy-last': await this.copyLastResponse(); break;
            case 'copy-all': await this.copyAllCode(); break;
            case 'export-chat': await this.exportFullChat(); break;
            case 'export-markdown': await this.exportMarkdown(); break;
            case 'show-stats': await this.showErrorStats(); break;
            case 'show-exports': await this.showExports(); break;
        }
    }
    ...
    async copyLastResponse() {
        if (this.lastCode) {
            ...
            this.showNotification('No code found', 'warning');
        }
    }

    async copyAllCode() {
        if (this.allCodeBlocks.length === 0) {
            this.showNotification('No code blocks found', 'warning');
            return;
        }
            ...
            ">
                ...
                ">
                    <div style="padding: 16px 20px; border-bottom: 1px solid #e0e0e0; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <button class="nj-modal-close" style="background: none; border: none; font-size: 24px; cursor: pointer; color: #999;">x</button>
                    </div>
                    <div style="padding: 20px; color: #333; font-size: 14px; line-height: 1.6; white-space: pre-wrap;">${content}</div>
                </div>
            </div>
        `;
